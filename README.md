...
| 何时选档 | 同一委派流程只选一次并复用结果，不为每个子代理重复取数。 |
| 不可用怎么办 | 角色不可用、模型不匹配或调用失败时，由主脑接手，不静默更换家族。 |
| 如何执行 | 通常使用 0–3 个 worker；独立任务才并行，主脑复核结果后交付。 |

</details>

分数和参考成本不代表你本机的表现，也不保证节省额度。
