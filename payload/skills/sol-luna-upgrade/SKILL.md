...
GPT-6 migration uses manifest schema 4 with a pinned model contract and unchanged ten-file agent inventory. Preserve old Daily/LKG files; new state lives under `gpt6-v4`. Transaction backups cover both managed roots. A verified exact candidate may be installed before publication only with explicit candidate authorization; final Release/tag/source identity must be checked afterward. If the Desktop still exposes GPT-5.6 custom roles, stop native acceptance and publication until reload; never substitute the shell CLI or direct model override as native proof.

Three-tier routing retains strict publication verification: verified live Radar, qualified same-generation cache, then explicit task-based basic routing. A Radar outage alone need not block installation or native acceptance in disclosed basic mode. Verify the installed routing contract and actual host roles; retain all source, ownership, model and transaction gates. Never describe basic-mode native acceptance as live Radar acceptance or import local evidence as an installed benchmark cache.
