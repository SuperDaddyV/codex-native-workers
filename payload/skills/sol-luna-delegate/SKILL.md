...
The `gpt6-v4` namespace rejects pre-verification GPT-6 Daily/LKG records as well as GPT-5.6 state. Keep old `gpt6-v3` state as historical evidence; never copy it into active state. Missing or mismatched publication evidence disables the affected view and its dependent general view. Do not substitute an unverified quality-only result.
