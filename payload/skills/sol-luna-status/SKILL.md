...
Schema 4 may additionally report `worker_models`, `cache_namespace`, `reference_policy_version`, and each worker model. For GPT-6 expect `gpt-6-sol`, `gpt-6-luna`, `gpt6-v3`, policy 2. These are configuration/cache identities, not observed native execution. Desktop host and shell CLI versions/capabilities must be reported separately when independently observed.
