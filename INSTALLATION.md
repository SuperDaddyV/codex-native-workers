...
The [v4.2.0 Stable contract](NATIVE_WORKERS_SETUP.md) governs current installation.
Execute the copy in the verified Release commit. Previous versions retain their
own immutable contracts; do not use a v4.1.4 contract with v4.2.0 source.

## 1. Choose the current Stable target

| Starting point | Action |
| --- | --- |
| New user | Use the **v4.2.0 Stable** prompt in the README. It adds Sol and Luna worker families and keeps your Coordinator choice. |
| Existing v4.1.4 or v4.2.0-rc1 user | Run that same prompt to upgrade the existing installation with preserved user content. |
| Already on v4.2.0 | A matching installation needs no rewrite or new backup. Check status if needed. |
...
GitHub's **Latest** release points to Stable v4.2.0. A Source code ZIP is not a
...
v4.2.0 manages the Global AGENTS block, owned Codex agent configuration, ten
...
| --- | --- |
| Source | Published `v4.2.0` Stable Release, its resolved exact 40-hex tag commit, clean detached checkout, remote tag checked again. A newer documentation commit is not this runtime source. |
...
| Read-only status | Ask `Check Sol/Luna status.` v4.2.0 normally reports `Status Healthy`, `Agents 10/10 Ready`, `Skills 3/3 Ready` and `leaf_config Ready`. These are configuration results. Legacy v4.1.4 has five Luna agents and a different status shape. |
...
of both v4.2.0 worker families.
...
| Only Luna appears | Check the version first: Legacy v4.1.4 installs no Sol workers. On v4.2.0, task routing may legitimately choose only Luna; status/model availability alone does not require using Sol. |
...
| Worker sees delegation tools | Strong recursive isolation is unsupported in v4.2.0. Tool visibility is not a successful nested call. Do not claim or depend on host isolation. |
...
roots. v4.2.0 recovery requires both roots and does not accept `--source-commit`;
...
The [v4.2.0 validation record](https://github.com/SuperDaddyV/codex-native-workers/releases/download/v4.2.0/native-workers-v4.2.0-validation.json)
